...
class DatasetViewer extends React.Component {
    ...
    render() {
        ...
        return (
            <div style={{background:`white`, maxWidth:`100%`, height:`100%`, overflowY:`scroll`, padding:10, borderRadius:`5px`, boxShadow:`0 4px 4px -2px gray`}}>
                ...
                <div>
                    {this.props.data.map((d, i) => (
                        <div key={i} className={`datasetpc data${i + 1}`} onClick={() => this.handleClick}><span className="datasetindexpc">{i + 1}.</span><span className="datasetnamepc">{d}</span></div>
                    ))}
                </div>
            </div>
        )
